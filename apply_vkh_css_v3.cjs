...
fs.copyFileSync(cssPath, cssPath + ".bak-vkh3-" + new Date().toISOString().slice(0, 19).replace(/[:.]/g, "-"));

const startMark = "/* ===== VIKHYATKAUSHIK hero";
const endMark = "/* ===== end VIKHYATKAUSHIK hero v2 ===== */";
...
const block = `/* ===== VIKHYATKAUSHIK hero v3 ===== */
:root{--vkh-ease:cubic-bezier(.16,1,.3,1)}
.vkh_hero{cursor:default}
.vkh_hero_contain{z-index:2;height:100%;flex-flow:column;justify-content:flex-end;align-items:flex-start;display:flex;position:relative}
...
/* bottom-right paragraph - same size and color as the navbar statement */
.vkh_hero_sub{z-index:2;align-self:flex-end;display:flex;position:relative;padding-bottom:calc(var(--site--margin)*.2)}
.vkh_hero_para2{max-width:30ch;color:var(--swatch--black-400);font-family:var(--_typography---font--primary-family);font-size:clamp(1.25rem,.9rem + 1.75vw,2.125rem);line-height:1.15;font-weight:500;letter-spacing:-.01em}
/* navbar statement swap (big paragraph replaces Hindi logo in hero) */
.navbar_left_contain{position:relative}
.vkh_nav_tagline{position:absolute;left:0;top:.35rem;opacity:0;max-width:22ch;color:var(--swatch--black-400);font-family:var(--_typography---font--primary-family);font-size:clamp(1.25rem,.9rem + 1.75vw,2.125rem);line-height:1.15;font-weight:500;letter-spacing:-.01em;pointer-events:none;margin:0;transition:opacity .5s ease,transform .6s var(--vkh-ease);transform:translateY(10px)}
body.vkh-nav-inhero .navbar_home{opacity:0 !important}
body.vkh-nav-inhero .vkh_nav_tagline{opacity:1;transform:translateY(0)}
/* preloader */
...
/* entrance states - MUST stay after the swap rules so the pre-boot
   hidden state wins while the preloader is up */
body:not(.vkh-boot-done) .vkh_nav_tagline,body:not(.vkh-boot-done) .vkh_hero_para2{opacity:0;transform:translateY(26px)}
.vkh_nav_tagline,.vkh_hero_para2{transition:opacity .9s var(--vkh-ease),transform .9s var(--vkh-ease)}
...
/* ===== end VIKHYATKAUSHIK hero v3 ===== */`;

css = css.slice(0, s) + block + css.slice(e + endMark.length);
fs.writeFileSync(cssPath, css);
console.log("CSS block v3 written OK");
